...
public class AireTriangle {
  ...
  private void createBinding() {
    // 1. Calcul du déterminant avec l'API fluente
    ...
    // 2. Liaison de area avec gestion de la valeur absolue (conditionnelle)
    area.bind(
        Bindings.when(determinant.greaterThanOrEqualTo(0))
            .then(determinant.divide(2.0))
            .otherwise(determinant.negate().divide(2.0)));

    // 3. Création de la StringExpression pour l'affichage formaté
    output =
