...
public class CalculatriceTriangle extends Application {
  ...
  public void start(Stage primaryStage) {
    // 1. Configurer le GridPane
    configGrille();

    // 2. Configurer les 6 sliders
    configSliders();

    // 3. Placer les titres P1, P2, P3
    ajouterLabelsPoints();

    // 4. Placer les sliders dans la grille
    ajouterSliders();

    // 5. Placer le label et le champ aire
    ajouterAire();

    // 6. Ajouter la zone de dessin
    ajouterPanneauDessin();

    // 7. Créer les bindings entre les composants et le modèle
    creerBindings();

    // 8. Créer la Scene, l'attacher au Stage, afficher
    Scene scene = new Scene(grille);
    primaryStage.setTitle("Calculatrice de Triangle");
  ...
  void ajouterLabelsPoints() {
    ...
    grille.add(labelP2, 0, 3); // Positionné juste au-dessus des coordonnées de P2
    grille.add(labelP3, 0, 5); // Positionné juste au-dessus des coordonnées de P3
  }
  ...
  void creerBindings() {
    // 1. Lier chaque slider au modèle (Liaisons unidirectionnelles car le modèle suit le slider)
    ...
    // 2. Lier le text du TextField au calcul de l'aire du modèle
    textFieldAire.textProperty().bind(modele.areaProperty().asString());

    // 3. Lier les coordonnées des lignes au modèle avec le facteur d'échelle multiplicateur de 50
